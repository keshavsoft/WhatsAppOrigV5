...
const getMessageData = (key, category = "responses") => {
  try {
    ...
    if (category === "responses" && messagesData.responses[key]) {
      ...
      category === "customMessages" &&
      messagesData.customMessages[key]
    ) {
...
const StartFunc = async (msg) => {
  ...
  const LocalNumbersData = await JSONFilePreset(
    "Data/mobiles.json",
    defaultData,
  );
  ...
  if (LocalFromNumber in LocalNumbersData.data) {
    ...
    LocalClientInfo.sendMessage(
      msg.from,
      LocalNumbersData.data[LocalFromNumber],
    ).then((PromiseData) => {});
  } else {
    ...
    if (!commandType) {
      ...
      if (errorData && errorData.type === "text") {
        ...
        msg.reply(
          "Sorry, I didn't understand that command. Type 'help' for available options.",
        );
      }
      return;
    }
    ...
    if (commandType === "responses") {
      ...
      if (responseData && responseData.type === "text") {
        if (messageBody === "Button") {
          ...
          const LocalClientInfo = getClientInfo();
          LocalClientInfo.sendMessage(msg.from, responseData.content)
            .then((PromiseData) => {
      ...
      if (mediaData) {
        ...
        try {
          if (messageBody === "SendMedia") {
            ...
            const LocalMediaUrl =
              "https://washtex5.keshavsoft.com/assets/image%20(1)-Bo3S5UVn.png";
            const media = await MessageMedia.fromUrl(LocalMediaUrl);
            await msg.reply(media);
          }
        } catch (error) {
          console.error(
            `Error sending fallback media for ${messageBody}:`,
            error,
          );
          msg.reply("Sorry, there was an error sending the media.");
        }
